...
... | no x≢y = no foo
  where
    foo : ¬ reg x ≡ reg y
    foo x≡y = contradiction (reg-inj x≡y)  x≢y
...
correctness-acc (plus e₁ e₂) m s s' r mv<r sx≡s'regmx =
  ...
  ≡⟨ cong (λ z → z acc) (sem-append (compile m r e₁) (([ STO r ] ++ compile m (suc r) e₂ ++ [ ADD r ])) s') ⟩
    sem-code (sem-code s' (compile m r e₁)) ([ STO r ] ++ (compile m (suc r) e₂ ++ [ ADD r ])) acc
  ≡⟨ cong (λ z → z acc) (sem-append [ STO r ] (compile m (suc r) e₂ ++ [ ADD r ]) (sem-code s' (compile m r e₁))) ⟩
    sem-code (sem-code (sem-code s' (compile m r e₁)) [ STO r ]) (compile m (suc r) e₂ ++ [ ADD r ]) acc
  ≡⟨ cong (λ z → z acc) (sem-append (compile m (suc r) e₂) [ ADD r ] (sem-code (sem-code s' (compile m r e₁)) [ STO r ])) ⟩
    (sem-code (sem-code (sem-code (sem-code s' (compile m r e₁)) [ STO r ]) (compile m (suc r) e₂)) [ ADD r ]) acc
  ≡⟨⟩
    update
      (sem-code
  ...
  where
    ...
    t1 : ℕ
    t1 =  sem-code (update (sem-code s' (compile m r e₁)) (reg r) (sem-code s' (compile m r e₁) acc)) (compile m (suc r) e₂) acc

    t1' = sem-code (update (sem-code s' (compile m r e₁)) (reg r) (source-sem s e₁) ) (compile m (suc r) e₂) acc

    t1≡t1' : t1 ≡ t1'
    t1≡t1' = cong (λ z →  sem-code (update (sem-code s' (compile m r e₁)) (reg r) (z) ) (compile m (suc r) e₂) acc) h1

    mx<sucr : ∀ x → m x < suc r
    mx<sucr x = s≤s (<⇒≤ (mv<r x))

    baz : (x : Var) → reg r ≢ reg (m x) → update (sem-code s' (compile m r e₁)) (reg r) (source-sem s e₁) (reg (m x)) ≡
